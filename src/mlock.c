...
int decode_file(FILE* input_file, off_t crypt_block_start, off_t eof_pos, uint8_t* b_file_nonce_prefix,
...
int minilock_decode(uint8_t* c_filename, uint8_t* b_my_sk, uint8_t* b_my_pk, uint8_t *c_override_out_name) {
    ...
    for (unsigned int i = 0; i < json_decrypt_info->u.object.length; i++) {
     ...
     //   fseeko(input_file, 0, SEEK_END); 
        ...
        if (decode_file(input_file, crypt_block_start, eof_pos, b_file_nonce, b_file_key, c_override_out_name)) {
            goto exit_decode_loop_on_failure;
        }
